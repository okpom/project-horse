...
func set_skill_for_slot(player_index: int, slot_index: int, skill: Skill, \
	target_slot_index: int) -> bool:
...
class SkillSlot:
	...
	func _init(_user, _skill: Skill, _source_slot_index: int, _target_slot_index: int, \
			   _target_player_index: int = -1, _target_entity = null):
		user = _user
		skill = _skill
